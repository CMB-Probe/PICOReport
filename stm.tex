...
\multicolumn{1}{l}{\parbox[t]{2in}{Tensor-to-scalar ratio $r^a$: $\sigma(r) = 1\times10^{-4}$ at $r = 0$; $r < 5 \times 10^{-4}$ at $5\sigma$ confidence level}} &
...
$^b$ Using the PICO $BB$ lensing power spectrum and $\tau$ with BAO from DESI or Euclid. The same constraint can be derived independently using clusters detected by PICO, if their redshift becomes available, and using  $\sigma_{8}$ from LSST.
}
\end{table}
