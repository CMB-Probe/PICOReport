...
(\S\,\ref{sec:survey_design} \comor{and Figure~\ref{fig:MissionDesignFigure}}), is supplemented with a margin of
...
\comor{replaced this paragraph with the orange sentences above.}
...
past the primary is suppressed by 80\,dB relative to the main lobe for
both co-pol and cross-pol beams. \comor{the rest of this paragraph can be pruned} 
Relative to the cross-Dragone, an
...
\comor{replaced all the material below, with what's above}
...
PICO's 555--799\,GHz channels (above the Niobium band gap) will use direct absorber bolometers. Ground and balloon experiments have deployed focal planes with hundreds of horn-coupled spiderweb bolometers. SPT-Pol deployed dual-polarized versions of direct absorber horn-coupled bolometers. \textit{Planck} used this style of bolometer, but with NTD-Ge thermistors instead of TESes (Figure~\ref{fig:DirectAbsorbing}). Filled arrays of detectors such as Backshort Under Ground (BUG) bolometers are also an option for these channels. The status of these efforts is summarized in Table~\ref{tab:suborbital}.
