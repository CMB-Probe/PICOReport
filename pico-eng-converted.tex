...
the massive baffles that the cross-Dragone does, and its compact
architecture fits a larger aperture within the shadow cone.
...
PICO focal plane is shown in Figure~\ref{fig:FocalPlaneMechanical} and
...
breakthrough is enabled by development and demonstration by suborbital
projects, which now commonly field arrays of $10^3$--$10^4$ detectors
...
\S\,\ref{sec:detector_development}). For all of these schemes, niobium
...
directly to absorber-coupled bolometers in the throat of a
...
experience at L2 \citep{Beyer2012}. For PICO, the primary contributor to noise is the
...
A more detailed description of the PICO noise model and its inputs is available in
...
using highly permeable cryogenic mu-metal shields that pull magnetic
fields away from the electronics and niobium housings that, once
superconducting, preserve a low-flux environment around the chips
\citep{Hui2018}. PICO will use these demonstrated techniques to
shield SQUID readout chips from the ambient magnetic environment
(significantly more benign at L2 than on Earth) as well as from fields
generated by on-board components (including, most notably, the cADR,
\S\,\ref{sec:cadr}).
...
Max power (MEV)&1526\,W (XX\% of available area)\\
...
\caption{PICO is compatible with the Falcon~9. {\color{Red}New figure expected.}\label{fig:InFairing}}
    \end{center}
  \end{minipage}
%
...
instrument elements (Figure~\ref{fig:InstrumentCAD}). A motor drives the
...
mode is telecom concurrent with science survey (1526\,W including 43\,\% contingency). Unused area in the
solar array plane affords significant margin for growth
(Figure~\ref{fig:Spacecraft}). The heritage power electronics are dual-string.
...
(Figure~\ref{fig:OpticalCoupling}). All have achieved background-limited performance in suborbital instruments with sufficient margin
on design parameters to achieve this performance in the more demanding
low background at L2.  Experiments have covered many of PICO's
observing bands between 27\,GHz and 270\,GHz
...
sky (Table~\ref{tab:bands}). Suborbital experiments have also demonstrated systematic control
...
PICO's 555--799\,GHz channels (above the niobium band gap) use
...
baseline design are:
\begin{enumerate}
\item extension of antenna-coupled bolometers down to 21\,GHz;
\item extension of 3-color antenna-coupled bolometers up to 462\,GHz;
...
0.61\,$\mu$K$_{\rm CMB}$\,arcmin aggregate map depth, which matches
the PICO CBE map depth, and affords $>40\,\%$ margin against the
0.87\,$\mu$K$_{\rm CMB}$\,arcmin baseline requirement
...
\citep{Bard1987}. PICO's
4.5\,m diameter V-groove assembly fits inside the
launch vehicle fairing. PICO
...
detector wafers to ensure adequate yield. 
...
is book-kept in WBS~6.  Mission operations (WBS~7), Ground Data Systems
...
test of the instrument with the spacecraft is book-kept in WBS~10
(ATLO).
