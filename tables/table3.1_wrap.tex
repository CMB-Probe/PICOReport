...
\begin{wraptable}[20]{r}{0.47\textwidth}
\vskip -0pt
\hfill
\begin{minipage}{0.47\textwidth}
\caption{\captiontext PICO makes efficient use of the focal area with multichroic pixels (three bands per pixel, \S\,\ref{sec:low_freq_det}). The sampling rate is based on the smallest beam (Table~\ref{tab:bands}), with 3 samples per FWHM at a scan speed $(360\degree/{\rm min})\sin(\beta=69\degree) = 336\degree/{\rm min}$. Scaling from suborbital experience, we anticipate that TES bolometers can support these sampling rates with $\sim4\times$ margin.\label{tab:focal_plane}}
\begingroup
