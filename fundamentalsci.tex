...
\parbox{3.3in}{
\caption{\captiontext
Cross-correlating PICO's lensing potential map with LSST galaxies can reach the important constraint of $f_{NL}<1\, (2\sigma)$ \comor{check value} if the data can be used at  angular scales $L > 6$ (solid black). A detection above that level indicates that inflation is driven by multiple fields; single-field inflation has $f_{NL}<1$ (green region). The \planck\ constraint is $f_{NL}>5\, (2\sigma)$\comor{check}. The cross-correlation will give $f_{NL}<??\, (2\sigma)$\comor{check} if LSST data is used only for $L>20$ and PICO's data for all $L>2$(dash). We assume three years of LSST data, magnitude limit of less than 27 in the $i$ band, galaxies at redshift $z<7$, and 50\% sky fraction. 
...
The axion is another dark matter candidate that is well motivated by string theory~\citep{Arvanitaki_etal}, and that is consistent with straightforward extensions of the standard model of particle physics~\citep{peccei,weinberg,wilczek}. For an axion mass in the intermediate range $10^{-30} < m_a< 10^{-26} $~eV, current measurements constrain its fraction to be equal to or less than 2\% of the total dark matter density, at $1\sigma$. If 2\% of the total dark content is made of axions, PICO's measurement of the $TT$, $TE$ and $EE$ spectra with additional constraints from the lensing reconstruction detect this species at between $7-13\sigma$, as shown in Figure~\ref{fig:axions}. This is an average improvement of a factor of 10 relative to \planck, and a factor of two relative to the combination of \planck and SO, respectively, profoundly shaping our understanding of the nature of dark matter and its small-scale clustering. 

\begin{figure}[t]
\begin{center}
\includegraphics[width=0.50\textwidth]{images/pico_cbe_errors.pdf}
\caption{\captiontext PICO will be able to detect an energy density as low as $2\%$ (the current $1\sigma$ upper bound from \planck) at between $7\sigma$ and $13\sigma$ confidence, for axions in the range $10^{-30}~\mathrm{eV} < m_a < 10^{-26}~\mathrm{eV}$. This will roughly double the constraining power of the Simons Observatory for the same axion masses.
\label{fig:axions} }
\end{center}
\vspace{-0.15in}
\end{figure}

%[13.11389417 10.35035967  7.64759865] ratio pico cbe
%[6.08000681 5.06950288 3.32743822] ratio SO
%[0.61085489 1.08113952 1.18210296] ratio planck
