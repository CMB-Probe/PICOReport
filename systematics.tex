...
Some of the PICO science goals attempt to detect extremely faint signals. The most ambitious one is to reach the nanoK-level 
...
CMB from space.  As WMAP\comor{?} and \planck~ demonstrated, the L2 environment offers excellent stability as well 
as the ability to observe large fractions of the sky on many time scales without interference from the Sun, Earth, or Moon.
The redundancy of observations allows the checking of consistency of results and an improved ability to calibrate and to correct systematic errors in post-processing analysis. \comor{there are several arguments here lumped in one sentence: suggest 
to make more explicit to PICO, and tie to the scan strategy. Also need to fold in the issue of 1/f (and absence of HWP.)}
