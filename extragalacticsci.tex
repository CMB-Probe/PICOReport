...
\caption{\label{fig:ReionizationPICO} Summary of constraints on the redshift and duration of reionization. The PICO forecasts are reported with 68\% and 95\% confidence-level contours PICO + CMB-S3 experinments and Planck  \planck{} large-scale data (blue/orange). The solid navy lines show the redshift and width of reionization at constant values of the IGM opacity and source efficiency. The \so{} constraints on these parameters are shown in Fig.~\ref{fig:patchy_sz_ma}. The \so{} predictions are compared to current exclusion limits for the time of reionization from \planck{} (green band;~\citealp{planck2018:parameters}), recent measurements of the global 21 cm signal assuming standard thermal properties (i.e., spin temperature much larger than the CMB temperature) of the IGM (yellow band;~\citealp{edges2017}), and Gunn Peterson trough from fully absorbed Lyman alpha in quasar spectra (gray band;~\citealp{Fan2006}), and to current upper limits on the duration of reionization from Planck and SPT data (green band;~\citealp{planck_reio:2016}).}
\end{figure}
