...
\begin{figure}
    ...
    \label{fig:allsky}
\end{figure}
%\noindent {\bf Outline:} {\em (target 2 pages)}
%\begin{itemize}
%\item Balance between magnetic fields, turbulence, and gravity is crucial in the formation of molecular clouds and filaments within molecular clouds.  
%\item What kind of measurements can we make?  (Refer to figure of merit).
%\begin{itemize}
%\item Determining the Alfven Mach number (relative orientation analysis, ordered field vs disordered field) {\color{red} Do we need velocity dispersion information- does it exist for the clouds we will measure- we probably need to say something about this}. 
%\item change in B strength vs gravity (comparing with velocity gradients, and DCF analysis).
%\item Note that these measurements depend on comparisons of PICO data with synthetic polarization observations of numerical models of star formation. {\color{red} What kind of comparisons? HROs? other?}
%\end{itemize}
%\item Our question: How is the formation of dense, gravitationally unstable structures within molecular clouds and subsequent efficiency of star formation affected by turbulence and magnetic fields?
%\item PICO will observe thousands of clouds with exquisite detail, can compare high mass/low mass clouds, more magnetized/turbulent vs less, old vs young, clouds that have been influenced by SN expansion, vs more quiescent environments.
%\item PICO will also make high resolution studies of magnetic fields and turbulence in filaments in 8 nearby clouds. PICO can observe how the magnetic field is affected or regulates gravitation collapse.  
%\begin{itemize}
%\item What role does the magnetic field play inside the filaments? 
%\item Mention that dust grain alignment efficiency makes it more difficult measure the magnetic fields in filaments, so that the high sensitivity and multiple frequency bands of PICO will be crucial to separating the polarized cloud dust emission from the filament dust emission.
%\item Quantify how many vectors we expect with PICO for these clouds vs Planck. {\color{red} How does this translate into efficacy of comparisons with models?}
%\end{itemize}
%\item Explain how PICO complement other telescopes (ALMA, SOFIA, OST, NearIR polarimeters, molecular line surveys).  Finish by discussing how all telescopes combined will allow to understand the formation of molecular clouds, cores, and star formation.
%\end{itemize}
